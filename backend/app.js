...
async function fetchTrackedOrder(orderNo, phone) {
    const response = await fetch(
        `${PF_TRACK_API}/api/orders/${encodeURIComponent(orderNo)}?phone=${encodeURIComponent(phone)}`
    );
...
function renderTrackedOrder(order) {
    ...
    box.innerHTML = `
        <div>
            <span class="eyebrow">ORDER #${order.order_no}</span>
            <h3 style="margin:4px 0 8px">${PF_TRACK_LABELS[status] || status}</h3>
            <span class="track-status ${status}">${PF_TRACK_LABELS[status] || status}</span>
        </div>
        <div class="track-meta">
            <div><span>Customer</span><b>${order.customer_name || "—"}</b></div>
            <div><span>Total</span><b>₹${Number(order.total || 0).toLocaleString("en-IN")}</b></div>
            <div><span>Order type</span><b>${order.fulfilment === "pickup" ? "Store pickup" : "Home delivery"}</b></div>
        </div>
    `;
}
...
$("#trackForm")?.addEventListener("submit", async e => {
    e.preventDefault();
    const orderNo = $("#trackOrderNo").value.trim();
    const phone = $("#trackPhone").value.trim();
    const box = $("#trackResult");
    try {
        box.classList.remove("hidden");
        box.innerHTML = `<p>Checking your order…</p>`;
        const order = await fetchTrackedOrder(orderNo, phone);
        renderTrackedOrder(order);
    } catch (err) {
        box.classList.remove("hidden");
        box.innerHTML = `<p style="color:#b42028">${err.message}</p>`;
    }
});
...
async function waitForOrderConfirmation(orderNo, phone) {
    if (successPollTimer) clearInterval(successPollTimer);
    let attempts = 0;
            ...
            if (!["new", "rejected"].includes(order.status)) {
                ...
                if (link) {
                    ...
                    link.onclick = () => {
                        ...
                        setTimeout(() => $("#trackForm")?.dispatchEvent(new Event("submit", {bubbles:true,cancelable:true})), 50);
                    };
                }
                const note = $("#successSummary");
                if (note) note.insertAdjacentHTML("beforeend", `<div style="margin-top:10px;font-size:12px"><b>✓ Order confirmed by Pizza Fire</b></div>`);
                clearInterval(successPollTimer);
            }
        } catch {}
        if (attempts >= 20) clearInterval(successPollTimer);
    };

    await check();
    successPollTimer = setInterval(check, 3000);
}

/* Patch the existing successful-order flow without disturbing checkout logic. */
const originalCheckoutSuccessText = $("#successText");
if (originalCheckoutSuccessText) {
    const observer = new MutationObserver(() => {
        const text = originalCheckoutSuccessText.textContent || "";
        const match = text.match(/Order\s+([^\s]+)\s+created/i);
        if (match && successOrderNo && successPhone) {
            successOrderNo = match[1];
            const link = $("#successTrackLink");
            link?.classList.add("hidden");
            waitForOrderConfirmation(successOrderNo, successPhone);
        }
    });
    observer.observe(originalCheckoutSuccessText, {childList:true,characterData:true,subtree:true});
}
